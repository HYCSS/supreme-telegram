...
hyCharades.model.skipCard = function() {
    var index = hyCharades.model.getCurrentTeamIndex();

    // Increment the current team's skipped cards
    hyCharades.model.teams[index].skipped_cards++;
};
...
hyCharades.startTurn = function() {
    ...
    // Initialize the 'next' and 'skip' buttons (after existing animations have completed)
    hyCharades.$.get('.next-btn').promise().done(function() {
        // Keep buttons hidden while initializing
        hyCharades.$.get('.next-btn, .skip-btn').hide();
        // Set the 'next' button text and state
        hyCharades.$.get('.next-btn')
            .text("Next Card")
            .focus();
        // Set the 'skip' button text and state
        hyCharades.$.get('.skip-btn')
            .text("Skip")
            .each(function() {
                    var index = hyCharades.model.getCurrentTeamIndex();

                    if (hyCharades.model.teams[index].skipped_cards >= 5) {
                        hyCharades.$.get('.skip-btn').addClass('disabled');
                    } else {
                        hyCharades.$.get('.skip-btn').removeClass('disabled');
                    }
                });
        // Fade the buttons in
        hyCharades.$.get('.next-btn, .skip-btn').fadeIn();
    });
...
hyCharades.setTimer = function() {
    ...
    seconds   = 7//hyCharades.model.seconds_per_turn;
...
hyCharades.nextCard = function(cards) {
    ...
    hyCharades.$.get('.next-btn').one('click', function() {
        ...
        // Update scoreboard
        hyCharades.updateScoreboard();
        // Remove click event listener from 'skip' button
    ...
    hyCharades.$.get('.skip-btn')
        .one('click', function() {
            ...
            // If the button has been clicked fewer than 5 times by the current team
            if (hyCharades.model.teams[index].skipped_cards < 5) {

                // Increment the current team's skipped cards
                hyCharades.model.skipCard();

                // If the team has now skipped 5 cards
                if (hyCharades.model.teams[index].skipped_cards === 5) {
                    // Set disabled state for skip button
                    hyCharades.$.get('.skip-btn').addClass('disabled');
                }

                // Remove click event listener from 'next' button
                hyCharades.$.get('.next-btn').off();

                // Draw next card
                hyCharades.nextCard(deck);

            // If the button has been clicked 5 or more times
            } else {
                // Set disabled state for skip button
                hyCharades.$.get('.skip-btn').addClass('disabled');
            }
...
hyCharades.endTurn = function() {
    // Remove click event listener from skip button and fade it out
    hyCharades.$.get('.skip-btn').off().fadeOut();
